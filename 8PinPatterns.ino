 /*     ---------------------------------------------------------------
 *  
 *  A few Simple LED animations: 8 LEDs, no shifter, multiple patterns to choose from. Mostly saving to reference codes.
 * 
 * Basic circuit setup:
 * Arduino ---> breadboard
 * 5V ---> red line
 * GND ---> GND line -- resistor -- LED (-) -- LED (+) <--- Arduino PINs 2-9
 * 
 */
 ...
 * Write the control pins to outputs
 */
void setup()
{
...
void loop()                     // run over and over again
{
  ...
  //oneOnAtATime();          //this will turn one LED on then turn the next one
                             ...
                             //along the line
  //KITT();                  //Same as above, but like KITT
 ...
 * oneOnAtATimeNoLoop() does exactly the same thing with more typing
 */
...
void oneOnAtATime(){
 ...
 /*
 * KITT() - Will light one LED then the next turning off all the others, then reverse
 */
void KITT(){
  int delayTime = 100; //the time (in milliseconds) to pause between LEDs
                       //make smaller for quicker switching and larger for slower
  ...
  for(i = 0; i <8; i++){
    digitalWrite(ledPins[i], HIGH);     //turn on LED #i
    delay(delayTime);
    digitalWrite(ledPins[i + 1], HIGH);     //turn on LED #i
    delay(delayTime);
    digitalWrite(ledPins[i], LOW); //turn off the LED we turned on last time
    delay(delayTime*2);
  }
